# Modifications for SQL Private IP

This repo is modified from https://github.com/GoogleCloudPlatform/golang-samples/tree/60e4b3700f89663d46ddd80fdca3a70c6326fa7d/cloudsql/postgres/database-sql


sql instance was created with only a private IP address


    gcloud sql databases create proxy-demo \
    --instance=run-demo-private
...
The Dockerfile was modified to copy in the sql proxy - and the entrypoint set to an init.sh script that starts proxy in BG, then app server


No sql connection is configured for Cloud Run - as it is using the version built into the container


    gcloud run deploy run-sql-demo-private --image gcr.io/ptone-misc/sql-sample \
      --set-env-vars INSTANCE_CONNECTION_NAME='ptone-misc:us-central1:run-demo-private' \
      --set-env-vars DB_USER='demo_user' \
      --set-env-vars DB_PASS='demo_password' \
      --set-env-vars DB_NAME='proxy-demo' \
      --vpc-connector=default-connector --vpc-egress=all
